...
const data = require('../xolentum-public-nodes.json')
...
function checkTestNode (node) {
  ...
  return request({ url: 'https://raw.githubusercontent.com/xolentum/public-nodes-json/main/xolentum-public-nodes.json', json: true })
    .then(data => {
  ...
  data.nodes.forEach(node => {
    return it(format('%s => %s:%s', node.name, node.url, node.port), function () {
      ...
      return checkTestNode(node)
        .then(check => {
          if (check) {
            return request({ url: format('%s://%s:%s/get_info', (node.ssl) ? 'https' : 'http', node.url, node.port), json: true, timeout: 4000 })
          } else {
            return this.skip()
          }
        })
